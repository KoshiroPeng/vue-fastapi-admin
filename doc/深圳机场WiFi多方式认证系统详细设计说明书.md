...
> 1. **不存储旅客业务数据**：MySQL 严禁建表持久化旅客任何业务隐私，包括但不限于旅客姓名、完整证件号、登机牌号、航班号、座位号、手机号、护照图像、MAC 上网追踪记录等。
...
项目内置 `scripts/auth_load_test.py`，从 Nginx 统一入口按固定到达率发起请求，覆盖登机牌和护照从业务验证、临时访客创建到 HACA 明确放行的完整后端链路。基线场景为 `50 RPS`、最大 `50` 个在途请求、持续 `60` 秒；脚本同时校验 HTTP 200 和 `networkAuthorized=true`，并输出错误率、完成吞吐量和 P50/P95/P99 延迟。默认判定门槛为错误率不超过 1%、P95 不超过 1 秒、P99 不超过 2 秒。

Mock 压测只验证本系统、Nginx、MySQL 和 Redis 的容量基线，不代表真实外部依赖满足指标。正式验收必须在两台应用服务器和真实 NCE、OCR、登机牌系统上执行，分别保存单链路和混合场景报告，并同步观察两台应用实例、MySQL、Redis、Nginx 及各上游的资源和错误指标。测试请求使用独立的脱敏 IP/MAC，避免单终端限流改变容量结果；防刷能力另按安全用例独立验证。
